...
class gaborFilters:
    ...
    def convolveImg(self, data, isPrint):
        ...
        dataVector = cv.CreateMat(1, data.width, cv.CV_8UC1)
        for i in range(0,data.height):
            for j in range(0,data.width):
                dataVector[0,j] = data[i,j]
            reshData = cv.Reshape(dataVector, 0, 70)
...
#________________________________________________________________________
    ...
    # def convolveImg(self, data, isPrint):
        ...
        # for i in range(0,data.height):
            # for j in range(0,data.width):
                # dataVector[0,j] = data[i,j]
            # cv.ShowImage("reshaped_test",cv.Reshape(dataVector, 0, 70))
            # cv.WaitKey()
        # for i in range(0,data.height):
