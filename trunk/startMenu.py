...
print "\t5 => do some classifications (SVN)"
print "\t6 => do some classifications (Knn)\n"
...
if(int(choice) == 1):
	...
	prep.doSmallManyGabors(datas[dataset], int(noComp1), True)
elif(int(choice) == 5):
	...
	classi.classifySVM(int(noComp), int(typeu), datas[dataset])
elif(int(choice) == 6):
	dataset = raw_input('classify h => hands vs garbage; r => rock & paper & scissors ...')	
	typeu   = raw_input('choose the data 1 => original images; 2 => PCA on initial images; 3 => multiple Gabor filters...')
	if(int(typeu) == 2):
		noComp = raw_input('number of components for PCA...')
	else:
	...
	datas  = {'r':'rock', 'h':'hands', 'p':'paper'} 
	classi = classifyHands(False)	
	classi.classifyKNN(int(noComp), int(typeu), datas[dataset])
